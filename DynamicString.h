...
private:
    ...
    //std::string *myArray;
    int len_ = 0;
    /** pointer to the underlying null-terminated dynamic array */
...
public:
     ...
     */
    DynamicString& toLower(int endPos);
    ...
    //Destructor frees up(deletes) any dynamically allocated memory used be the class (i.e.the char array)
    ~DynamicString() { delete[] myArray; } // dealocate data from dynamic array.
